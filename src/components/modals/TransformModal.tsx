...
export default function TransformModal({ isOpen, onClose, onApply, currentAdjustments }: TransformModalProps) {
  ...
  const renderControls = () => (
    <div className="modal-adjustments-pane z-10 flex h-full w-80 shrink-0 flex-col border-l border-border-color bg-bg-secondary">
      <div className="flex shrink-0 items-center justify-between border-b border-border-color p-4">
        ...
        <button onClick={handleReset} data-tooltip={t('modals.transform.resetTooltip')} className="modal-icon-action">
          <RotateCcw size={18} />
        </button>
      </div>
  ...
  return (
    <div className={`app-modal-backdrop ${show ? 'opacity-100' : 'opacity-0'}`} onMouseDown={onClose}>
      <AnimatePresence>
        {show && (
          <motion.div
