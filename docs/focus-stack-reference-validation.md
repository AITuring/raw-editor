...
- 流式 mosaic 的色调场只从当前拍摄组已经拥有的重叠估计。最终不再对所有 ownership 边界
  追加一次宽范围的 group gain；边界两侧可能是不同笔画，不能把它们当作曝光测量。
...
在 `RAW_EDITOR_REFERENCE_RENDER_SCALE=0.20` 下，84/84 张源图都通过参考注册和局部形变，生产
流式 renderer 输出的有效覆盖为 `8027×2003`，且有效矩形每个像素都有 source ownership。验证过的
局部包括顶部装裱线、中央人物/树干区域和右侧题跋/印章区域；重点检查的是明暗接缝、错位笔画和
分析网格边，而不是只看整图缩略图。

最终展示图若使用 PS 参考的低频/几何并只叠加 RAW 结果的高频细节，必须明确标为
“reference-matched composite”。它适合作为对照交付，不应冒充纯 RAW renderer 输出，也不能作为
后续算法回归的唯一基准；纯算法结果仍应通过上述 harness 重新生成并检查覆盖率与 ownership。
