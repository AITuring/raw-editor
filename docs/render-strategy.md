...
5. 桌面 JPEG/PNG/TIFF 主图导出按横向 tile 完成一个最多 2048 行的带状 RGBA8 缓冲后，依次交给可选的
   ...
   `DynamicImage`。JPEG 使用 `zenjpeg 0.8.4` baseline 4:4:4、Annex K 量化表与 fixed Annex K
   Huffman 单遍逐行输入；输出先写同目录临时文件，完整编码、元数据处理和取消检查都成功后才原子
   替换目标路径。
...
- Rust 单元测试：验证 camelCase 序列化、预览/导出边界、60MP 空蒙版计划、`Arc::ptr_eq` 共享、
  ...
  参考一致、水印与旧完整帧混合逐像素一致，以及 JPEG Q50/Q75/Q92 相对旧编码器的平均 RGB
  误差、文件体积和质量单调性。
- `npm run synthetic-export:bench`：默认编码 9504×6336 PNG；`RAW_EDITOR_BENCH_FORMAT` 可选
  ...
  `RAW_EDITOR_BENCH_WATERMARK=1` 加入确定性水印；宽高仍可通过 `RAW_EDITOR_BENCH_WIDTH` 和
  `RAW_EDITOR_BENCH_HEIGHT` 覆盖。

## 合成 60MP 编码基准

2026-08-09 在 Apple M2 Max（12 核）、32 GB、macOS 15.6.1 上运行。测试构造并重复使用与生产
GPU 拼带相同上界的 2048 行 RGBA8 缓冲，再逐行经过生产用 resize/watermark 和 JPEG/PNG/TIFF
编码器写入临时文件；RSS 由独立线程每 10 ms 采样。它不初始化 WGPU、不解码 RAW，也不判断画质，
因此不能替代真实相机验收。

| 格式与变换            | 输入 → 输出尺寸       |     耗时 |      输出大小 |      峰值 RSS | 生产者带状缓冲 | 旧完整 RGBA8 缓冲 |
| --------------------- | --------------------- | -------: | ------------: | ------------: | -------------: | ----------------: |
| PNG，无变换           | 9504×6336 → 9504×6336 |   816 ms |   2,934,463 B |  84,590,592 B |   77,856,768 B |     240,869,376 B |
| TIFF，无变换          | 9504×6336 → 9504×6336 |   170 ms | 361,305,558 B |  87,441,408 B |   77,856,768 B |     240,869,376 B |
| JPEG Q92，无变换      | 9504×6336 → 9504×6336 | 1,898 ms |  81,882,505 B | 342,999,040 B |   77,856,768 B |     240,869,376 B |
| JPEG Q92，缩放 + 水印 | 9504×6336 → 4096×2731 | 1,304 ms |  17,866,661 B | 143,982,592 B |   77,856,768 B |     240,869,376 B |
...
- WebP、JXL、AVIF 与 Android 导出仍使用完整 CPU 编码图。桌面 JPEG 已改用通过 60MP harness 的
  `zenjpeg 0.8.4`，但该版本完成编码时会同时持有压缩扫描数据和组装后的 JPEG；高熵全尺寸测试的
  峰值 RSS 因而为 342,999,040 B。它消除了完整 RGBA8 输出帧，但还不是压缩码流零拷贝写文件，
  也不能把生产者缓冲下降等同于整个进程峰值必然下降。
- JPEG/PNG 在保留 EXIF 时会暂时把“已压缩输出文件”读回内存交给现有元数据写入器；它不恢复完整
