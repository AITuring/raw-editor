...
6. `color`/`luminance` 以 2048px core 加有限 halo 生成：halo 精确等于 grow 的方形形态学半径加
   ...
   15,897,612 B。每个最终 CPU bitmap 仍为完整尺寸；范围匹配源由第 21 项继续收敛。
...
10. 几何 warp 对 RGB32F/RGBA32F 输入直接借用现有底层浮点切片，采样器同时支持三、四通道；只有
    ...
    staging；第 19、20 项另为符合条件的导出增加逐行/逐带输出，而不改变其他消费者的返回类型。
...
19. 全尺寸导出在浮点 RGB/RGBA 源、有效几何变换且无镜头模糊/精细旋转时，使用
    `GeometryWarpRows` 将共同的几何采样核
...
20. `GpuInputSource` 现允许生产者直接填充一个上传带；`GeometryWarpRows` 将 90°/270° 粗旋转和
    水平/垂直翻转作为精确坐标置换融合到该带。0°/180° 仍直接生成目标行；换轴方向按源图每 64 列
    保存一次与旧完整 warp 相同的浮点累加检查点，再生成最多 64 列的几何小块并转置到上传 staging。
    9504×6336 经 90° 旋转后的检查点为 11,328,768 B，转置 scratch 与上传 staging 各为
    3,244,032 B；相比旧路径在旋转或翻转期间同时保留两张 RGBA32F 的 1,930,199,040 B 逻辑峰值，
    新路径合计 17,816,832 B，减少 1,912,382,208 B（99.1%）。变换后的裁剪、所有粗方向与翻转组合
    均和旧路径逐个 RGBA16F 值一致；精细旋转和镜头模糊仍回退，范围蒙版则由下一项按需采样并可与
    该流式导出并存。
21. `RangeMaskSource` 将颜色/明度距离匹配从完整 `DynamicImage` 解耦；`GeometryWarpSampler` 直接借用
    灰度/RGB/RGBA 的 8/16/32 位存储，按输出行批量读取实际坐标，并只为访问到的源行每 64 列惰性
    建立一次与旧 warp 相同的浮点累加检查点。9504×6336 全图扫描的检查点上限为 11,328,768 B，取代
    963,477,504 B 完整 RGBA32F warped source，减少 952,148,736 B（98.8%）；局部 tile 只分配被访问
    源行的检查点。RAW 为保持原语义仍先做一次默认 CPU 显示变换并缓存完整 RGB32F 匹配源，但不再
    追加 RGBA32F warp。该范围源缓存与旧 AI 全量 warp 缓存互斥；后者会接管并复用已完成默认显示
    变换的 RAW 源，建立全量 warp 后清除范围源，避免两张 722,608,128 B RGB32F 图长期并存。复杂几何、
    裁剪、四个粗方向、翻转、颜色/明度、组合模式及 tile halo 均由逐像素回归锁定，范围蒙版也不再关闭
    第 19、20 项的流式几何导出。
...
| 颜色/明度几何匹配源（不计借用/缓存的输入图）           |  963,477,504 B（918.8 MiB） | 11,328,768 B（10.8 MiB）上限 |   952,148,736 B |
| 可流式导出的几何 RGBA32F 输出 + GPU 上传 staging       |  968,343,552 B（923.5 MiB） |       4,866,048 B（4.6 MiB） |   963,477,504 B |
| 90°/270° 几何输出、正交置换与 GPU 上传 CPU 峰值        | 1,930,199,040 B（1.80 GiB） |     17,816,832 B（17.0 MiB） | 1,912,382,208 B |
...
CPU 峰值中移除；粗旋转/翻转再通过带级生成、精确检查点与有界转置避免第二张完整置换结果。范围蒙版
通过独立的按需采样器复用相同几何核，不再触发该回退；精细旋转和镜头模糊仍物化完整输出。
...
180,652,032 B 降到三张 2366×2366 tile 的 16,793,868 B（减少 90.7%）。颜色/明度的 RGBA32F
warped source 已由最多 11,328,768 B 的检查点替代；这些数字仍不包含 RAW 默认显示匹配源、AI 解码源
和最终 CPU bitmap。各项生命周期不同，不能与表内其他减少量直接相加为进程峰值。
...
- `npm run render-strategy:check`：锁定四级 IPC 合同、单底图缓存、dummy mask、活动蒙版 tile texture、
  RAW RGB 分配交接、RGB32F 原位预处理/显示变换、有界 RGBA16F 带状上传、几何 row/band source、
  正交变换检查点、范围蒙版按需几何源与导出回退条件、CPU 蒙版 `Arc` 共享、首个加法子蒙版接管输出、程序化/画笔 2048px 分块、颜色/明度与
...
- Rust 单元测试：验证 camelCase 序列化、预览/导出边界、60MP 空蒙版/活动蒙版 tile 计划、蒙版局部
  ...
  裁剪后的逐行 RGBA16F 与完整帧逐值一致、四个粗方向/四种翻转组合的带状输出与变换后裁剪逐值一致、
  全部十种 `DynamicImage` 存储的按需几何采样与完整 warp 逐像素一致、60MP 宽行浮点累加及范围蒙版
  与流式导出并存、复杂二次重采样回退和 60MP 几何缓冲计划、
...
- `npm run synthetic-orthogonal-geometry-output:bench`：固定在相同源上执行 90° 粗旋转与水平翻转；默认
  `streamed` 路径以检查点和 64 列转置块填充旋转后的 6336×9504 上传带，设置
  `RAW_EDITOR_ORTHOGONAL_GEOMETRY_OUTPUT_BENCH_MODE=materialized` 可运行旧的完整 warp、完整旋转、
  完整翻转与带状转换。两种模式使用相同的 RSS 采样与 RGBA16F 稀疏哈希。
...
- `npm run synthetic-range-source:bench`：默认让 `GeometryWarpSampler` 从 9504×6336 RGB32F 源按需
  生成颜色范围 bitmap；设置 `RAW_EDITOR_RANGE_SOURCE_BENCH_MODE=full` 可先物化旧完整 RGBA32F
  warped source。两种模式都在共同源分配后读取 RSS 基线，以 2 ms 间隔采样并计算相同的稀疏蒙版哈希；
  宽高仍可通过 `RAW_EDITOR_BENCH_WIDTH` 和 `RAW_EDITOR_BENCH_HEIGHT` 覆盖。
...
完整帧逐值一致。该基础 harness 不创建 WGPU texture、不执行 shader/编码，也不读取 RAW；它自身
不包含粗旋转/翻转，正交扩展由下节单独测量。精细旋转、镜头模糊、非浮点源和
`color`/`luminance` 蒙版仍走完整帧回退。

## 合成 60MP 正交几何输出基准

同机以独立测试进程在上述几何参数之后固定执行 90° 粗旋转与水平翻转，输出尺寸为 6336×9504。
`materialized` 模式依次生成完整 RGBA32F warp、旋转结果和翻转结果，再转换到 64 行上传 staging；
`streamed` 模式以生产路径相同的 64 列浮点检查点、几何小块与带内转置直接生成最终方向的 RGBA16F。
两次运行都在 RGB32F 源和 3,244,032 B 上传 staging 已存在后读取相同 RSS 基线。

| 90° 几何 + 水平翻转 → GPU 上传路径     |   耗时 |      基线 RSS |        峰值 RSS |        RSS 增量 |   逻辑 CPU 峰值 | 稀疏 RGBA16F 哈希  |
| -------------------------------------- | -----: | ------------: | --------------: | --------------: | --------------: | ------------------ |
| 旧：完整 warp、旋转、翻转后带状转换    | 588 ms | 737,050,624 B | 2,667,528,192 B | 1,930,477,568 B | 1,930,199,040 B | `c616c5f75cc28bf6` |
| 新：检查点 + 64 列小块直接转置到上传带 | 264 ms | 737,050,624 B |   753,811,456 B |    16,760,832 B |    17,816,832 B | `c616c5f75cc28bf6` |

进程峰值减少 1,913,716,736 B（71.7%），附加 RSS 减少 99.13%；逻辑 CPU 峰值减少
1,912,382,208 B（99.1%）。耗时减少 324 ms（55.1%），来源包括不再写入两张完整置换图以及连续小块
采样，不能外推为真实 RAW 解码或完整导出加速。相同哈希证明上传边界收到相同半精度像素；Rust 回归
另覆盖 0°/90°/180°/270°、水平/垂直翻转的全部组合、变换后裁剪、尾带及行对齐 padding。该 harness
仍不创建 WGPU texture、不执行 shader/编码，也不读取 RAW；检查点 11,328,768 B、最大转置小块
3,244,032 B 和上传 staging 是有界 CPU 资源，不表示 GPU 输入 texture 已分块。
...
2026-09-08 在 Apple M2 Max（12 核）、32 GB、macOS 15.6.1 上，以独立测试进程构造并触碰
9504×6336 RGB32F 图样。`full` 模式作为完整物化对照，先生成 RGBA32F geometry warp；`sampled`
模式用生产路径相同的 `GeometryWarpSampler` 按输出行成批采样。两者随后共用当前的并行颜色距离
匹配器。RSS 基线均在共同的 722,608,128 B RGB32F 源已存在后读取，运行期间共同保留最终
60,217,344 B 灰度 bitmap。

| 颜色范围几何匹配源路径         |   耗时 |      基线 RSS |        峰值 RSS |        RSS 增量 | 检查点像素数据上限 | 稀疏蒙版哈希       |
| ------------------------------ | -----: | ------------: | --------------: | --------------: | -----------------: | ------------------ |
| 完整物化 RGBA32F warped source | 739 ms | 728,088,576 B | 1,756,987,392 B | 1,028,898,816 B |      963,477,504 B | `18e331080673aa4b` |
| 按需采样 + 惰性浮点累加检查点  | 658 ms | 727,924,736 B |   804,995,072 B |    77,070,336 B |       11,328,768 B | `18e331080673aa4b` |

进程峰值减少 951,992,320 B（54.2%），源分配后的 RSS 增量减少 951,828,480 B（92.5%）；逻辑匹配源
上限减少 952,148,736 B（98.8%）。本次单次运行观察到耗时减少 81 ms（11.0%），但两种模式都使用
当前并行匹配器，不能把它当作迁移前提交与当前提交的精确耗时对比，也不外推为真实照片或所有参数下
的固定提速；可复用结论是输出哈希一致，且按需路径不会重新分配完整 RGBA32F warp。
颜色与明度共享同一采样器和坐标映射，二者的完整像素一致性另由含复杂几何、裁剪、粗方向、翻转、
组合模式及 tile halo 的 Rust 回归锁定。该 harness 不读取 RAW、不初始化 WGPU，也不包含最终编码；RAW
路径仍需保留一次默认 CPU 显示变换后的完整 RGB32F 匹配源，因此这些数字不是端到端应用峰值。
...
- 全分辨率导出仍同时需要已解码/变换输入、一张完整 RGBA16F GPU 输入 texture 和最多 64 行的
  ...
  旋转、镜头模糊和非浮点源仍物化该输出；颜色/明度蒙版已改用按需几何源，不再关闭流式路径；
  90°/180°/270°粗旋转及水平/垂直翻转已经融合到逐带生产者。
...
- WebP 仍使用完整 CPU 输入与 libwebp YUVA picture，但已消除额外 ARGB 图和完整压缩输出缓冲；
  ...
  编码依赖会在内部返回完整压缩 `Vec`；常见及正交几何导出输出、范围蒙版匹配源已经收敛，下一阶段
  优先评估精细旋转/镜头模糊的二次重采样节点及可替换的 JXL/AVIF 分块编码器。当前锁定的
  rawler 上游没有公开行式/分块
...
- 活动蒙版 GPU texture、cache/caller 所有权、程序化/画笔 scratch 与颜色/明度 grow/feather 已按
  ...
  一张全尺寸灰度 bitmap，RAW 颜色/明度匹配仍缓存一张默认显示变换后的完整 RGB32F 源，AI 子蒙版
  仍保留完整灰度解码源；范围匹配不再追加完整 RGBA32F warp。缓存预算只阻止超大条目长期驻留；后续
  若要继续降低蒙版峰值，需要改变最终 bitmap、RAW 显示匹配源或 AI 解码源的存储/消费合同，而不只是
  继续缩小 scratch。
