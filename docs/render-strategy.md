...
19. 全尺寸导出最初在浮点 RGB/RGBA 源、有效几何变换、无镜头模糊/精细旋转/粗旋转/翻转，且没有
...
20. `GpuInputSource` 现允许生产者直接填充一个上传带；`GeometryWarpRows` 将 90°/270° 粗旋转和
    ...
    均和旧路径逐个 RGBA16F 值一致；精细旋转、镜头模糊及依赖完整 warped source 的范围蒙版仍回退。
...
CPU 峰值中移除；粗旋转/翻转再通过带级生成、精确检查点与有界转置避免第二张完整置换结果。精细旋转、
镜头模糊和依赖完整 warped source 的范围蒙版仍回退。
...
- `npm run render-strategy:check`：锁定四级 IPC 合同、单底图缓存、dummy mask、活动蒙版 tile texture、
  ...
  正交变换检查点与导出回退条件、CPU 蒙版 `Arc` 共享、首个加法子蒙版接管输出、程序化/画笔 2048px 分块、颜色/明度与
...
- 全分辨率导出仍同时需要已解码/变换输入、一张完整 RGBA16F GPU 输入 texture 和最多 64 行的
  ...
  旋转、镜头模糊、非浮点源和依赖完整 warped source 的颜色/明度蒙版仍物化该输出；90°/180°/270°
  粗旋转及水平/垂直翻转已经融合到逐带生产者。
...
- WebP 仍使用完整 CPU 输入与 libwebp YUVA picture，但已消除额外 ARGB 图和完整压缩输出缓冲；
  ...
  编码依赖会在内部返回完整压缩 `Vec`；常见及正交几何导出输出已经收敛，下一阶段优先消除范围蒙版
  对完整 warped source 的依赖，并继续评估精细旋转、可替换的 JXL/AVIF 分块编码器。当前锁定的
  rawler 上游没有公开行式/分块
