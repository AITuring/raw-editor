...
- 欢迎页提供统一“图像堆栈”入口，可一次选择 2–30 张受支持的 RAW 或位图；进入工作台后再选择景深堆栈或全景拼接，不在首页重复暴露工作流按钮。
- 堆栈工作台使用无外层留白的全窗口布局，预览画布占据主要空间，工作流、对齐模型和源图层集中在右侧；源图层支持鼠标拖拽、键盘拖拽及上下按钮调整基准顺序。
- 自动对齐优先在相似与仿射模型之间稳健选择，通过双向 RANSAC、全分辨率局部相关搜索和亚像素峰值细化降低残余错位；透视模型必须通过稳定性检查，避免画布外推造成拉伸或局部重影。
