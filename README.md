...
- 欢迎页提供统一“图像堆栈”入口，可一次选择 2–200 张受支持的 RAW 或位图；进入工作台后再选择景深堆栈或全景拼接，不在首页重复暴露工作流按钮。超过 30 张时自动使用低内存大批量路径，候选关系同时参考用户顺序与文件名自然顺序，并对水墨留白等低纹理画面补充低阈值特征检测，避免文件选择器乱序或角点过少造成整组断连。
- 堆栈工作台使用无外层留白的全窗口布局，预览画布占据主要空间，工作流、对齐模型和源图层集中在右侧；源图层支持鼠标拖拽、键盘拖拽及上下按钮调整基准顺序。
- 自动对齐优先在相似与仿射模型之间稳健选择；30 张以内继续使用双向 RANSAC、全分辨率局部相关搜索和亚像素峰值细化，超过 30 张则在自适应分辨率的灰度分析图上完成邻域匹配，再按需逐张解码原图参与最终融合。大批量自动全景使用平移稳定模型，避免微小旋转、缩放或透视误差在上百张长链中累积；明确改变机位时仍可选择透视模型。
- 支持自动、透视、圆柱、球面和仅位置对齐模型；自动结果不稳定时可以切换模型并重新对齐。
- 同机位景深堆栈使用局部清晰度评分与多尺度频带融合；检测到明显取景位移时改用连续最优接缝，避免跨画面逐像素混合产生双影。全景拼接对低频曝光与高频纹理分别融合，在平滑亮度过渡的同时保留边缘细节。
