...
- ✅ 裁切工具已按 Camera Raw 的日常工作流重构：支持预设、自定义及可锁定宽高比、横竖比例切换、
  ...
  避免再次出现“控件有数值但画面不变”。“约束到图像”会按当前裁剪、粗旋转、翻转、投影、透视、
  镜头畸变和缩放的完整逆变换计算安全区域。未裁切预览先缩至显示分辨率再变换，连续拖动只保留
  当前任务和最新请求，裁切工作区不再并发生成不可见的主预览。面板会显示实际输出像素尺寸，方向键
  可按源像素微移裁剪框（`Shift` 为 10 像素），并提供可恢复进入状态的“取消”与保留结果的“完成”。
  `C` 进入裁切、`A` 拉直；裁切重置不再误清几何或镜头参数。
...
`cargo check --lib --locked`、71 项默认执行的
