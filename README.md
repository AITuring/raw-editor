...
- 从图库多选 2–30 张图像进入独立图像堆栈工作台。
- 使用旋转感知二进制特征、互相最近邻匹配、归一化 DLT 和双向 RANSAC 完成稳健对齐。
- 支持自动、透视、圆柱、球面和仅位置对齐模型；镜头畸变导致结果不稳定时可以切换模型重新对齐。
- 景深堆栈使用局部清晰度评分与多尺度频带融合；全景模式使用投影变换、最优接缝搜索和局部多频段融合，保留纹理高频并平滑亮度过渡。
- 结果可预览、重新对齐、保存为 TIFF，并保留本地处理 sidecar 关联。
