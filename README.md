...
- 欢迎页提供统一“图像堆栈”入口，可一次选择 2–200 张受支持的 RAW 或位图；进入工作台后再选择景深堆栈或全景拼接，不在首页重复暴露工作流按钮。超过 30 张时自动使用低内存、顺序感知的大批量对齐路径，源图按拍摄顺序排列可显著减少匹配时间。
- 堆栈工作台使用无外层留白的全窗口布局，预览画布占据主要空间，工作流、对齐模型和源图层集中在右侧；源图层支持鼠标拖拽、键盘拖拽及上下按钮调整基准顺序。
- 自动对齐优先在相似与仿射模型之间稳健选择；30 张以内继续使用双向 RANSAC、全分辨率局部相关搜索和亚像素峰值细化，超过 30 张则在自适应分辨率的灰度分析图上完成邻域匹配，再按需逐张解码原图参与最终融合。透视模型必须通过稳定性检查，避免画布外推造成拉伸或局部重影。
...
- 合成结果使用缓存文件提供最高约一亿像素的高质量预览，支持滚轮缩放、拖拽平移、适合窗口、100% 查看和专注预览。预览与保存绑定同一个结果 ID，并共享规范的 16 位显示编码 sRGB 像素；导出的 TIFF 嵌入同一 sRGB ICC，避免浮点 TIFF 被查看器按线性 RGB 解释而造成显示差异，同时保留本地处理 sidecar 关联。
