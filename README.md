# RAW Editor

<p align="center">
  <img src="public/logo.svg" alt="RAW Editor 标志" width="128" height="128" />
</p>

一个面向个人摄影工作流的、跨平台、非破坏性 RAW 编辑器。

本项目以 [RapidRAW](https://github.com/CyberTimon/RapidRAW) 为上游基础，在接受并遵守
AGPL-3.0 的前提下继续开发。现阶段不追求 AI 功能，也不追求与 Adobe Camera Raw
像素级完全一致；目标是先实现一套画质可靠、响应流畅、可以每天使用，并且核心摄影
工作流与 Camera Raw 基本一致的非 AI RAW 编辑器。
上游来源与再发行声明见 [NOTICE](NOTICE)。

## 支持平台

RAW Editor 当前只面向桌面平台开发、测试和发布：

- **macOS**：首要开发和发布平台。
- **Windows**：维持 x64 与 ARM64 构建和安装包。
- **Linux**：维持 x64 与 ARM64 构建，并提供 AppImage、DEB 和 RPM 安装包。
- **Android：暂停支持**。Daily RAW 1.0 阶段不再开发、测试或发布 Android 版本，也不提供
  APK/AAB 安装包。仓库中的 Android 脚手架暂时保留，仅供未来重新评估，不代表当前可用或受支持。

## 项目目标

首个正式目标是完成 **Daily RAW 1.0**：

- 支持常见 RAW 文件的可靠导入、预览、编辑和导出，首要测试机型为 Sony α7R V。
- 从打开 RAW 到导出成片，全程保持非破坏编辑，绝不修改原始文件。
- 覆盖 Camera Raw 的核心非 AI 摄影流程：
  - 相机配置文件、白平衡和基础影调；
  - 曲线、HSL、颜色分级和黑白；
  - 锐化、亮度降噪、颜色降噪；
  - 镜头校正、裁剪、旋转、拉直和透视；
  - 画笔、线性渐变、径向渐变、颜色范围和明度范围蒙版；
  - 修复、仿制、预设、历史、快照、复制/同步设置；
  - 胶片栏、批量编辑、后台导出和色彩管理。
- 在 60MP RAW 上仍能流畅调整参数、查看 100% 细节并稳定导出。
- macOS 作为首要开发平台，同时维持 Windows 和 Linux 的可构建性。

“与 Camera Raw 基本一致”指功能结构、编辑语义和日常操作流程一致，不代表复刻 Adobe
界面，也不要求使用同一参数值后得到完全相同的像素。Adobe 的相机配置文件、色彩算法
和处理模型并不开源，本项目将通过公开标准、开源算法和自己的回归样片建立独立画质基线。

## 当前基础版本

当前仓库已经合并 RapidRAW 上游代码，并保留 `rapidraw-upstream` 远程用于里程碑边界同步。
应用身份已切换为 RAW Editor，默认启用 `basic` 模式：保留 RAW 导入、预览、全局调整、裁剪、
非 AI 蒙版、sidecar 和导出主链路。旧版浏览器编辑器的 `ImageState` 参数模型通过
`src/basic/legacyAdjustmentAdapter.ts` 映射到 RapidRAW 的原生 `Adjustments`，因此后续可以
继续复用旧页面的参数语义，而不再依赖旧的 WebGL/浏览器 RAW 解码器。

应用现已固定为纯本地产品：没有账号或登录、社区与在线预设、云端处理、远程 AI connector、
在线地图、自动更新检查和遥测，也不会把照片、EXIF、编辑参数或提示词上传到服务器。公开开源
依赖不在此限制内：构建时可以从公共源获取依赖，用户明确启用本地模型能力时也可以下载
ONNX Runtime 或公开模型到应用私有数据目录；推理由本机完成。依赖与模型也可以预先打包，
准备完成后核心编辑流程不要求联网。

编辑数据统一写入应用私有数据目录的 `sidecars-v1`，不再在照片文件夹旁生成 `.rrdata` 等内部
文件。首次遇到旧版相邻 sidecar 时会先原子迁移到私有目录，确认成功后删除旧文件。Daily RAW 1.0
格式契约见 [`schemas/sidecar-v1.schema.json`](schemas/sidecar-v1.schema.json)。

本地运行：

当前 Vite 8 构建支持 Node.js 20.19+ 或 22.12+ 的对应版本线；开发与 CI 推荐固定 Node.js 24 LTS，
Node.js 21 不受支持。

```bash
npm install
npm run dev              # 仅启动 Vite 前端
npm run start            # Tauri 2 开发窗口
```

`VITE_RAW_EDITOR_MODE=full` 只用于后续调试上游完整面板；日常开发保持默认的 basic 模式。
需要构建或打包本地 ONNX 功能时，显式设置 `RAW_EDITOR_ENABLE_AI_RUNTIME=1`；默认构建不会
预取或打包 ONNX Runtime。该开关只允许获取公开运行时，不会启用任何云端产品能力。

### 应用身份与语言

- 产品名：`RAW Editor`
- 应用标识：`io.github.AITuring.RawEditor`
- 当前开发版本：`0.1.0`
- 项目主页：<https://github.com/AITuring/raw-editor>

### 品牌与图标资源

应用标志采用相机轮廓与非均分六叶光圈组合。可缩放的纯 SVG 主资源位于
[`public/logo.svg`](public/logo.svg)，macOS Liquid Glass 分层源稿位于
[`design/logo/macos-liquid-glass`](design/logo/macos-liquid-glass)。Tauri 打包所需的 macOS
`icns`、Windows `ico`、Linux PNG、Windows 磁贴以及保留脚手架中的 Android/iOS 图标，均由
该 SVG 生成并存放在 [`src-tauri/icons`](src-tauri/icons)。更新品牌标志时，应从 SVG 重新生成
整套平台资源，不要在 SVG 中嵌入或放大 PNG。

首次启动会按照系统语言自动选择最接近的界面语言，并在设置中持久化用户选择。英语、
简体中文和繁体中文作为当前优先维护语言；上游已有的其他语言资源继续保留。新增界面文案
必须同时补齐英语与中文，并通过 `npm run i18n:check` 后才能合并。

### 当前进度（2026-09-08）

当前处于 **M0 基础整合完成、M1 RAW 与色彩基线建设阶段**。用户授权的 `DSC05363.ARW` 已建立
α7R V `lossy-arw` 单样片真实管线与本机 60MP 性能基线；其余光照、ISO、曝光和 RAW 压缩模式仍
等待扩充。无需新增样片即可确定性验证的工程工作已完成：

- ✅ 合并 RapidRAW 上游代码，保留 `rapidraw-upstream` 远程，并完成 Tauri 2 原生启动基线。
- ✅ 完成 RAW Editor 的应用标识、版本、窗口标题、桌面包元数据和基础 EXIF 软件标识调整。
- ✅ 采用相机与非均分六叶光圈组成的新标志，并由纯 SVG 统一生成 Web、macOS、Windows、
  Linux 以及保留脚手架中的 Android/iOS 图标资源；仓库内不再保留旧标志位图。
- ✅ 完成中英文首启自动选择、设置持久化、语言代码归一化和翻译完整性校验。
- ✅ 固定为本地软件，移除账号、登录、社区、在线预设、云端处理、远程 connector、在线地图和更新检查链路。
- ✅ 保留公开开源依赖与模型的可选下载/本地打包能力；所有照片处理和模型推理均在本机完成。
- ✅ 增加 `npm run local-only:check` 回归边界，阻止重新引入产品云端地址、前端网络 API、
  shell 权限、社区命令，或在公开资源下载模块以外新增 Rust 网络客户端。
- ✅ 复用旧版 `raw-editor` 页面的 `ImageState` 参数语义，适配到 RapidRAW 原生 `Adjustments`。
- ✅ 清理 RapidRAW 遗留的 TypeScript 类型与 lint 问题，`tsc --noEmit` 和零警告 ESLint 成为合并门槛；
  上游动态 UI/Tauri payload 暂保留显式 `any` 例外，新建的配置、i18n 和公共类型代码继续严格禁止。
- ✅ 增加 α7R V/rawler 相机矩阵、Sony Lensfun、确定性本地降噪、全尺寸导出和 ICC 往返验收。
- ✅ 为 JPEG、PNG、TIFF、WebP 和 JPEG XL 导出嵌入固定 sRGB v4 ICC，并记录第三方来源与哈希。
- ✅ 冻结 Daily RAW 1.0 sidecar schema v1；支持 v0 迁移、未知字段保留、原子写入、上一版备份、
  损坏恢复、未来版本拒绝和评分范围校验。
- ✅ 覆盖私有 sidecar 迁移/复制，以及“单张失败不丢弃后续结果”的批量导出聚合回归。
- ✅ 常规验收不携带派生图片，使用内存生成的确定性图样；测试合同与真实 RAW 清单见
  [`tests/acceptance`](tests/acceptance/README.md)。
- ✅ 为已解码全分辨率图像、蒙版、几何预览、缩略图几何和 LUT 缓存增加 LRU 条目上限与
  估算内存预算；超预算条目继续参与当前处理但不驻留缓存，避免 60MP 工作流持续累积内存。
- ✅ 高倍率缩放和平移停稳后只渲染带 overscan 的可见 ROI，CPU 回退通过版本化二进制区块
  协议更新画面，原生 WGPU 直接更新对应纹理区域；全分辨率变换底图使用 `Arc` 共享，避免深拷贝。
- ✅ 原图对比、未裁切预览、几何/镜头预览和蒙版覆盖统一改为原生二进制 IPC；WebView 只在展示
  边界创建并回收 Blob URL，并通过 `npm run preview-transport:check` 阻止核心编辑链路退回整图 Base64。
- ✅ 完成 RAW → WGPU → WebView/原生预览 → 文件导出的首轮颜色空间审计；CPU 端统一使用标准
  sRGB 传递函数，修复 LinearRaw gamma 模式将 0.5 错误按 3.0 指数解码的问题，并通过
  `npm run color-contract:check` 锁定 CPU/WGSL、RAW 分支、GPU 输出与导出 ICC 边界。完整契约与
  已知缺口见 [`docs/color-pipeline.md`](docs/color-pipeline.md)。
- ✅ 普通 JPEG/PNG/TIFF 输入现会读取嵌入 RGB ICC，并通过纯 Rust `moxcms` 转换为编辑器的
  编码 sRGB 合同；4 MiB profile 上限、Display P3 数值基线、畸形/缺失 profile 回退和 TIFF
  元数据解码器回归均有确定性内存测试。核心 WebView JPEG 预览、预设、LUT 与图库缩略图统一
  嵌入 sRGB v4 ICC，原生 WGPU 继续声明 sRGB surface 交由系统合成器映射到当前显示器。
- ✅ 裁切工具已按 Camera Raw 的日常工作流重构：支持预设、自定义及可锁定宽高比、横竖比例切换、
  六种可实际切换的裁剪参考线（画布右键或 `Option/Alt+V` 循环）、±45° 精细旋转、画线拉直和基于
  本地图像直线检测的自动拉直、90° 旋转及水平/垂直翻转。几何面板提供关闭、自动、水平、垂直、
  完全和引导六种 Upright 模式；自动、水平、垂直和完全模式在 Rust 图像分析端融合灰度与 RGB
  分通道边缘，以长连续边作为透视主证据，并让分布在多个画面区域、方向一致且彼此正交的短边参与
  旋转判断，因此印章、窗格和瓷砖等低明度差细节也能成为参考。轴向 RANSAC 会隔离水平与垂直证据，
  只有跨画面的收敛关系明显优于平直模型时才写入透视参数；断续纹理和文字笔画不会再被拼成贯穿画面
  的假透视线，证据不足时保持图像不变。引导模式允许直接在照片上绘制 2–4 条应当水平或垂直的参考线
  并实时写入同一组几何参数，而不是只改变界面状态。手动菜单中的投影、垂直、水平、
  旋转、纵横比、缩放和 X/Y 偏移均进入同一套 Rust 逆向采样矩阵；其中“投影”负责压缩画面边缘拉伸，
  与光学面板的镜头畸变保持独立。每个参数都
  同步作用于未裁切预览、蒙版坐标、裁切安全区、sidecar 和全尺寸导出，并由合成网格像素回归锁定，
  避免再次出现“控件有数值但画面不变”。旋转、透视、投影和镜头畸变不再隐式放大画面来隐藏边缘；
  “约束到图像”默认关闭，允许在固定画布内保留变换覆盖不到的区域，裁切预览以中性棋盘明确显示这些
  空白，PNG、WebP、TIFF 和 JPEG XL 等支持 alpha 的导出继续保留透明度。启用约束时会按当前裁剪、
  粗旋转、翻转、投影、透视、镜头畸变和缩放的完整逆变换计算安全区域；再次关闭会恢复自动管理的
  最大画布，同时保留用户手工缩小或移动过的裁框。拖动精细旋转或几何滑杆时只移动画面，裁框保持
  稳定并在松手后一次求解，避免连续收缩和跳动。未裁切预览先缩至显示分辨率再变换，连续拖动只保留
  当前任务和最新请求，裁切工作区不再并发生成不可见的主预览；开发模式下的 Strict Mode 生命周期
  也不会再永久销毁预览队列并造成裁切页白屏。面板会显示实际输出像素尺寸，方向键可按源像素微移
  裁剪框（`Shift` 为 10 像素），并提供可恢复进入状态的“取消”与保留结果的“完成”。`C` 进入裁切、
  `A` 拉直；顶部“重置”会同时还原裁框、方向、精细旋转和全部几何参数，但保留独立光学面板的镜头
  校正参数，“取消”仍恢复进入裁切时的状态。
- ✅ 快速预览、半分辨率编辑、源像素级 ROI 和全分辨率导出现已成为显式四级合同；前端选择层级，
  Rust 复核交互/ROI/源尺寸边界。预览缓存只保留单一目标底图，无蒙版使用 1×1 dummy texture，
  活动蒙版不再拼接第二份 CPU 上传数组，分析结果与未变换原图通过 `Arc` 共享，消除多类整图重复
  缓冲。确定性 60MP 尺寸前后基线、限制和仍存在的峰值来源见
  [`docs/render-strategy.md`](docs/render-strategy.md)。
- ✅ 活动蒙版 GPU array 现复用与图像处理一致的 2048px tile 加 128px overlap，每个 tile 只从现有
  灰度 bitmap 上传对应 ROI，WGSL 使用局部坐标采样。9504×6336 单层 `R8` texture 从
  60,217,344 B 收敛到最多 5,308,416 B，减少 54,908,928 B（91.2%）；32 层上限从
  1,926,955,008 B 降至 169,869,312 B。本机 Metal 像素回归另跨越 2048px seam 验证零蒙版逐字节
  不变、白蒙版正确应用调整。CPU 侧每层最终灰度 bitmap 仍存在；后续条目继续收敛其所有权和生成 scratch。
- ✅ 蒙版缓存与当前 render 现在通过 `Arc<GrayImage>` 共享同一像素分配，命中和写入缓存都不再深拷贝；
  常见的首个加法子蒙版直接成为最终合成缓冲，初始黑色整图不再预分配，前置减去/相交也不再生成
  注定无法改变黑色底图的临时 bitmap。9504×6336 单层缓存 + caller 的逻辑 CPU 像素从
  120,434,688 B 降至 60,217,344 B（50%）。同机独立进程合成对照的 RSS 增量从 120,750,080 B
  降至 60,489,728 B，减少 60,260,352 B（49.9%），稀疏哈希一致。每个活动蒙版仍需一张完整 CPU
  灰度 bitmap；本项本身只收敛所有权，不把最终结果描述成 tile 化存储。
- ✅ 非空画笔、仿制、修复与流量画笔从首个子蒙版起按 2048px tile 生成；radial、linear 和 all 在已有
  合成结果后也逐 tile 生成并立即就地执行添加、减去或相交。tile 保留完整画布坐标与独立输出原点，
  避免负坐标截断导致 seam 偏移；反转、透明度及跨横纵 seam 的逐像素结果均与旧完整帧一致。60MP
  后续程序化子蒙版的逻辑 scratch 从 60,217,344 B 降至 4,194,304 B（减少 93.0%），最终 bitmap +
  scratch 从 120,434,688 B 降至 64,411,648 B（减少 46.5%）。画笔最坏情况下的子蒙版结果 + 单笔
  stroke scratch 都被限制为两个 tile；加上最终图后从 180,652,032 B 降至 68,605,952 B（减少
  62.0%）。同机 60MP 独立进程对照的 scratch RSS 增量从 60,522,496 B 降至 4,751,360 B，减少
  55,771,136 B（92.1%），稀疏哈希一致。颜色、明度的跨像素过滤由下一项继续收敛；AI 蒙版仍保持完整帧。
- ✅ 颜色范围与明度范围蒙版现按 2048px core 加精确 halo 生成：grow 的方形形态学半径与
  `imageproc` 有限 Gaussian 核的 `ceil(2σ)` 半径相加，tile 只在完整画布边缘截断，过滤后仅把 core
  就地合成到最终 bitmap。旋转、翻转、粗方向、正负 grow、feather、添加/减去/相交、反转和透明度均
  已跨横纵 seam 与旧完整帧逐像素一致。在 9504×6336 和 UI 支持的最大 grow/feather 下，halo 为
  127px，单个扩展 tile 最多 2302×2302；过滤期三张灰度 scratch 从 180,652,032 B 降至
  15,897,612 B，减少 164,754,420 B（91.2%）。同机 60MP 独立进程以 grow=2、feather=1 同时覆盖
  形态学和模糊，对照 scratch RSS 增量从 186,531,840 B 降至 18,055,168 B，减少 168,476,672 B
  （90.3%），稀疏哈希同为 `22b2b2054060aa43`。每个最终 CPU 灰度 bitmap 仍存在；下一项继续消除
  当时供范围匹配使用的完整 warped source。
- ✅ 颜色/明度范围匹配现通过 `GeometryWarpSampler` 按输出行成批读取实际需要的几何像素，不再预先
  物化完整 RGBA32F warped source。采样器直接借用 `DynamicImage` 的 8/16/32 位、灰度/RGB/RGBA
  存储，并按源行每 64 列惰性保存一次与旧 warp 相同的浮点累加检查点；复杂几何、裁剪、粗方向、翻转
  及 tile overlap 的结果均与旧完整帧逐像素一致。RAW 仍先执行一次默认 CPU 显示变换并缓存完整
  RGB32F 源，但不再为范围匹配追加 RGBA32F warp；范围蒙版也不再阻止符合条件的全尺寸几何导出使用
  `GeometryWarpRows`。9504×6336 的检查点像素数据上限从 963,477,504 B 降至 11,328,768 B，减少
  952,148,736 B（98.8%）。本机 Apple M2 Max（12 核）、32 GB、macOS 15.6.1 的确定性 RGB32F
  对照中，完整物化路径为 739 ms / 1,756,987,392 B 峰值 RSS，按需路径为 658 ms / 804,995,072 B；源分配后
  的 RSS 增量从 1,028,898,816 B 降至 77,070,336 B（减少 92.5%），稀疏蒙版哈希同为
  `18e331080673aa4b`。RAW 默认显示源缓存与旧 AI 全量 warp 缓存互斥；需要全量 warp 时会先接管并复用
  已处理的 RAW 源，完成后回收范围源，避免两张约 689 MiB 的 RGB32F 图长期并存。该 harness 不解码
  RAW、不初始化 WGPU；两种模式共用当前并行匹配器，因此耗时只比较匹配源策略，不代表迁移前代码或
  真实相机的端到端性能。
- ✅ 主体、前景、天空、深度与快速橡皮擦 AI 子蒙版现只解码一次 base64 灰度源，并以 2048px core
  加精确 halo 完成几何映射和后续过滤；深度选区先就地计算，再把 `image` Gaussian 的有限核半径与
  通用 grow/feather 半径相加，过滤后只合成 core。首个无过滤加法蒙版仍直接成为最终 bitmap，避免
  为分块额外预分配输出。旋转、翻转、粗方向、正负 grow、深度/通用双重 feather、五种 AI 类型、
  添加/减去/相交、反转和透明度均已跨横纵 seam 与旧完整帧逐像素一致。60MP、UI 最大参数下深度
  halo 为 159px，三张过滤 scratch 从 180,652,032 B 降至最多 16,793,868 B，减少 163,858,164 B
  （90.7%）。同机 halo=3 的独立对照在共同保留解码源与最终 bitmap 后，scratch RSS 增量从
  605,700,096 B 降至 70,107,136 B（减少 88.4%），逻辑 scratch 从 180,652,032 B 降至
  12,656,748 B（减少 93.0%），哈希同为 `849aa825f4208721`。解码后的 AI 源与每层最终 CPU bitmap
  仍是完整尺寸；本项只消除了完整的变换结果与跨像素过滤临时图。
- ✅ RAW 三通道浮点开发结果现将 rawler 的 `Vec<[f32; 3]>` 原分配直接交给 `ImageRgb32F`，不再同时
  创建带恒定 alpha 的 RGBA32F；去伪色/锐化与默认 RAW 显示变换也会在已有 RGB32F 上原位执行。
  GPU 上传继续保持 RGBA16F 合同，但直接从 RGB32F/RGBA32F 转换，不再先克隆一张 RGBA32F CPU
  staging。9504×6336 的开发结果交接逻辑峰值从 1,686,085,632 B 降至 722,608,128 B，减少
  963,477,504 B（57.1%）；长期开发帧从 963,477,504 B 降至 722,608,128 B，减少 240,869,376 B
  （25%）。同机独立进程对照的峰值 RSS 从 1,697,824,768 B 降至 734,134,272 B，减少
  963,690,496 B（56.8%），附加 RSS 从 963,919,872 B 降至 229,376 B，稀疏 RGB 哈希同为
  `b509617393486fb7`。该基准从已开发 RGB 中间图开始，不包含 RAW 文件、mosaic 解包、去马赛克、
  WGPU texture 或真实相机画质；RAW 解码器内部与最终开发帧仍是完整尺寸。
- ✅ GPU 输入纹理继续保持完整尺寸和现有 shader 的全图坐标合同，但 RGBA16F CPU 转换改为最多 64 行、
  按 256-byte 行对齐的有界带状上传。9504×6336 的 CPU staging 上限从 481,738,752 B 降至
  4,866,048 B，减少 476,872,704 B（99.0%）；输入 GPU texture、当时的完整几何 warp 输出和 rawler
  内部开发帧仍然存在，因此该上传阶段本身不冒充几何节点或 GPU 显存已经 tile 化。逐行浮点/16 位
  转换、带宽算术和 shader/纹理合同由 Rust 单测与 `npm run render-strategy:check` 锁定；下一项继续
  收敛符合条件的几何输出。
- ✅ 全尺寸导出中的单次重采样几何路径现由 `GeometryWarpRows` 直接写入同一 64 行 RGBA16F 上传带；
  裁剪只选择虚拟完整 warp 的连续行，不再先物化 RGBA32F 后裁切。9504×6336 的 CPU 几何输出 + 上传
  staging 从 968,343,552 B 降至 4,866,048 B，减少 963,477,504 B（99.5%）。同机独立进程对照的
  峰值 RSS 从 1,704,247,296 B 降至 740,605,952 B，减少 963,641,344 B（56.5%）；188 ms 与
  179 ms 基本持平，稀疏 RGBA16F 哈希同为 `8ea96650e5e63553`。该基础路径当时不包含粗旋转/翻转；
  精细旋转、镜头模糊和非浮点源仍保留旧完整帧回退，颜色/明度蒙版则已由上面的按需采样源解除该
  回退条件。GPU 输入 texture 也仍是完整尺寸，因此本项只收敛符合条件的导出 CPU 几何输出。
- ✅ `GpuInputSource` 与 `GeometryWarpRows` 现支持直接填充整个 64 行上传带，把 90°/180°/270° 粗旋转
  及水平/垂直翻转作为精确坐标置换融合进生产者；换轴时按源图每 64 列保存一次与旧逐行浮点累加一致的
  11,328,768 B 检查点，只生成最多 3,244,032 B 的几何转置小块。9504×6336 经 90° 旋转并水平翻转时，
  旧完整 warp/旋转/翻转 + staging 的逻辑 CPU 峰值为 1,930,199,040 B，新路径为 17,816,832 B，减少
  1,912,382,208 B（99.1%）；同机独立进程峰值 RSS 从 2,667,528,192 B 降至 753,811,456 B，减少
  1,913,716,736 B（71.7%），588 ms 降至 264 ms，稀疏 RGBA16F 哈希同为 `c616c5f75cc28bf6`。
  四个方向、四种翻转组合、变换后裁剪、尾带和行对齐 padding 均与旧路径逐值一致；颜色/明度范围
  蒙版现在可与该流式边界并存，精细旋转和镜头模糊仍未进入。
- ✅ 全尺寸几何 warp 对 RGB32F/RGBA32F 源直接借用底层浮点切片，只有其他存储格式才回退到
  RGBA32F 转换，不再为 9504×6336 RGB32F 输入额外 staging 一张 963,477,504 B 的 RGBA32F 源图。
  同机合成 60MP 连续对照中，旧 staging 路径为 291 ms / 2,613,788,672 B 峰值 RSS，新借用路径为
  180 ms / 1,649,917,952 B，峰值减少 963,870,720 B（36.9%），稀疏输出哈希一致。warp 输出仍是
  物化接口的一张完整 RGBA32F 图，因此这一历史步骤只收敛输入；上面的逐行导出路径会绕过该输出。
- ✅ 桌面端 JPEG/PNG/TIFF 主图导出现由 WGPU tile 汇集为最多 2048 行的带状缓冲，再按需经过
  `zenresize 0.3.1` 的逐行 Lanczos ring、单行水印混合并直接交给编码器；resize 和 watermark 不再
  迫使这三种格式恢复完整 RGBA8 CPU 帧。输出仍通过同目录临时文件原子发布，取消或编码失败不会
  留下半张目标文件。9504×6336 的生产者缓冲上限为 77,856,768 B，相比完整 RGBA8 帧少
  163,012,608 B。
- ✅ 水印添加与默认水印直接沿用 `my-watermark` 水印页的交互和
  `src/assets/default-watermark.png` 原始资产
  （SHA-256 `4db6f91d61c973c4a580bc0710c9115a9b84da9c46e52f1db1e2de129bf7f8c0`）：开启水印后立即显示
  真实默认水印缩略图，点击即可用 PNG/JPEG/WebP/TIFF/GIF 替换，并可一步恢复默认。默认位置为
  居中、不透明度 80%；水印开关仍默认关闭，避免未确认就改变导出。内置水印由 Vite 预览资产和
  Rust `include_bytes!` 共用稳定标识，打包后不依赖开发服务器 URL 或用户机器文件路径；完整帧与
  JPEG/PNG/TIFF 逐行导出继续复用同一后端缩放、定位和 alpha 混合逻辑。自定义水印仅接受文件
  选择器明确授权的图像，验证大小和像素边界后转为 PNG，并按内容哈希原子写入应用私有
  `watermarks` 目录；WebView 只获准预览该窄目录，因此重新启动后仍可显示，不需要开放主目录。
- ✅ 桌面 JPEG 改用 `mozjpeg 0.10.13` 的 baseline 4:4:4 逐扫描行接口，压缩数据直接写入同目录
  临时文件，不再在收尾阶段同时保留压缩 scan 和组装后的完整 JPEG。Q50/Q75/Q92 高频图样回归
  继续约束相对旧 `image` 编码器的平均 RGB 误差、输出体积和质量单调性；sRGB v4 ICC、逐行数
  校验、取消检查和原子发布保持完整。编码调用整体包在 unwind 边界内，将底层 libjpeg 错误转换为
  导出失败并清理临时文件。
- ✅ 保留元数据的桌面 JPEG/PNG 先生成最多一个 JPEG APP1 可表示的 TIFF/EXIF 小载荷，分别在
  JPEG 编码开始时写入 APP1、在 PNG 编码头写入标准 `eXIf` chunk；16 位 TIFF 则在条带编码期间
  直接写 IFD0、ExifIFD、GPSIFD 和标准目录指针。三种格式都不再把已压缩输出整文件 `read` 回内存、
  清空并重写。合成源的 Make/Software/拍摄时间/GPS 保留与 `stripGps` 删除已有往返测试，另覆盖
  TIFF → TIFF 元数据复制；`npm run render-strategy:check` 锁定编码期写入和禁止整文件读回的边界。
- ✅ 桌面 WebP 保持原有 libwebp 质量标尺、alpha 和最终文件字节不变，但有损编码现直接把 RGBA/RGB
  输入导入 YUVA picture，避免 libwebp 在编码期间再持有一张完整 ARGB 工作图；压缩回调直接写入
  同目录临时文件，sRGB v4 ICC 再用固定 64 KiB 缓冲做 RIFF 两遍重写并原子发布，不再同时持有
  `WebPMemory` 和第二份完整 ICC 组装结果。RGB/RGBA 的 Q50/Q75/Q90/Q100 字节一致、ICC 往返、
  截断 RIFF、取消和权限保留均有回归测试。
- ✅ CPU 回读型导出不再额外分配两张完整尺寸显示 texture；按 9728×6400 的 GPU 对齐尺寸计算，
  两张 RGBA8 显示 surface 的逻辑占用减少 498,073,592 B。导出结束后，processor 与输入 texture
  合计达到 512 MiB 高水位即主动回收；大尺寸显示 processor 切换到 CPU 导出时也会按滞回规则收缩。
- ✅ 扩展可重复的 9504×6336 合成大图编码基准 `npm run synthetic-export:bench`，并加入
  `RAW_EDITOR_BENCH_METADATA=1` 的编码期 EXIF 路径。在同一台 Apple M5（10 核）、32 GB、
  macOS 26.5 上，旧 JPEG 路径为 1,137 ms / 330,317,824 B 峰值 RSS；新直接写文件路径为
  384 ms / 90,308,608 B，峰值减少 240,009,216 B（72.7%），输出只增加 155,660 B（0.19%）。
  开启合成 EXIF 后为 388 ms / 90,619,904 B，证明元数据路径没有恢复与压缩文件大小成正比的
  读回缓冲。基准实际分配 77,856,768 B 生产者带状缓冲，但不包含 RAW 解码或 GPU 处理，因而
  只证明 RGB 行管线的稳定性和内存边界，不代表 α7R V 画质或端到端性能。
- ✅ 新增 `npm run synthetic-webp:bench` 的 `memory`/`file` 同输入对照。同机同一测试二进制下，
  9504×6336 RGBA、Q90 的旧内存路径为 11,178 ms / 31,316,612 B / 1,371,684,864 B 峰值 RSS；
  新 YUVA + 文件路径为 11,128 ms / 31,316,612 B / 1,130,758,144 B，峰值减少 240,926,720 B
  （17.6%）。50 ms（0.4%）耗时差属于近乎持平，输出逐字节一致。该合成基准不含 RAW 解码和
  WGPU，不能替代真实相机端到端结论。

Sony α7R V 60MP ARW 已有一个用户授权的 ISO 125、25 秒、Sony 有损压缩样片基线，覆盖真实解包、
开发、预览/全尺寸 JPEG、ICC、局部画质检查与本机耗时；结论和边界见
[`tests/acceptance/sony-a7rv-60mp-baseline.md`](tests/acceptance/sony-a7rv-60mp-baseline.md)。它没有色卡、
配对 ACR 输出，也不能代表日光、钨丝灯、高 ISO、欠曝、高饱和光、lossless-L 或 uncompressed，
因此不会用合成图或这一个长曝光样片冒充完整机型结论。无需新增样片的 RGB 输入 ICC 与日常 SDR
显示链路已经闭环；CMYK/Gray/CICP-only 输入、自定义显示 profile 和软打样仍属于后续能力。
四级渲染策略、RAW 开发 RGB 零拷贝交接/原位预处理/有界 RGBA16F 带状上传、活动蒙版 GPU tile、CPU
共享所有权、程序化/画笔分块生成、颜色/明度精确 overlap 与按需几何源、AI 蒙版变换/过滤 tile、几何
warp 浮点输入借用及常见/正交全尺寸导出的逐行/逐带输出、桌面端 JPEG/PNG/TIFF tile-to-encoder、逐行 resize/watermark、
内置默认水印、GPU 高水位回收、JPEG/WebP 压缩码流直接写文件、JPEG/PNG/TIFF 编码期 EXIF 和合成
大图 harness 已经闭环。
WebP 仍接收一张完整 CPU 输入并保留 libwebp 的 YUVA picture，但不再复制完整 ARGB 图或缓冲完整
压缩输出；RAW 解码器内部 mosaic 与开发帧、每个活动蒙版的最终 CPU bitmap、AI 蒙版解码源、精细
旋转/镜头模糊等二次重采样回退以及 JXL/AVIF 导出仍走完整帧路径。当前锁定的 rawler 上游没有公开
行式/分块 `Intermediate` 开发接口；下一阶段优先评估精细旋转/镜头模糊的二次重采样节点，再继续评估
上游 RAW API、替换开发器或可替换的 JXL/AVIF 分块编码器。待新 α7R V 样片到位后，再扩充 RAW → GPU → 文件
的场景画质、交互耗时和峰值基线。

本轮已通过 `npm run typecheck`、`npm run lint`、`npm run i18n:check`、`npm run crop-transform:check`、
`npm run color-contract:check`、`npm run local-only:check`、`npm run preview-transport:check`、
`npm run preview-resolution:check`、`npm run render-strategy:check`、`npm run watermark-contract:check`、
`npm run build`、`npm run synthetic-export:bench`、`npm run synthetic-webp:bench`、
`npm run synthetic-geometry:bench`、
`npm run synthetic-geometry-output:bench`、
`npm run synthetic-orthogonal-geometry-output:bench`、
`npm run synthetic-mask:bench`、`npm run synthetic-mask-compose:bench`、
`npm run synthetic-range-mask:bench`、`npm run synthetic-range-source:bench`、`npm run synthetic-ai-mask:bench`、
`npm run synthetic-raw-handoff:bench`、`npm run gpu-mask:check`、
`cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`、
`cargo check --manifest-path src-tauri/Cargo.toml --lib --locked`、
`cargo test --manifest-path src-tauri/Cargo.toml --lib --locked`（187 项默认执行的 Rust 单元/回归测试）和
普通 Clippy。Clippy 1.97 的 `-D warnings` 严格门槛仍会报告现存的 batch geometry、panorama 与焦点堆栈
告警，本次范围源代码没有新增 Clippy 告警，因此不把严格门槛记作通过。默认忽略十九项：六项需要本地
授权或外部图像 fixture，十二项是手动大图/内存 benchmark，另一项需要本机 GPU；本轮已显式执行跨 tile 的
Metal 蒙版像素回归，几何阶段的 `borrowed`/`staged` 以及蒙版阶段的 `shared`/`cloned`、
`tiled`/`full`、范围与 AI 蒙版 `tiled`/`full`、RAW 交接 `reused`/`expanded`、几何输出
基础与正交 `streamed`/`materialized` 对照都保持相同稀疏输出哈希。`git diff --check` 也保持通过。

## 1.0 范围

### 必须完成

#### RAW 与色彩管线

- RAW 解包、黑电平、白电平、坏点处理和方向信息。
- Bayer 与 X-Trans 基础支持。
- 相机白平衡、自定义色温/色调和白平衡拾色器。
- 去马赛克、高光恢复、相机色彩矩阵或 DCP 配置文件。
- 32 位 scene-linear 内部工作空间。
- 显示器 ICC 转换、软打样基础能力和导出 ICC 嵌入。
- 高质量全分辨率分块导出，预览和导出使用同一参数语义。

#### 全局调整

- Profile、Auto、曝光、对比度、高光、阴影、白色、黑色。
- 纹理、清晰度、去雾、自然饱和度、饱和度。
- 参数曲线、点曲线以及红、绿、蓝独立曲线。
- HSL/颜色混合器、颜色分级、黑白混合和基础相机校准。
- 锐化数量、半径、细节和蒙版。
- 亮度降噪、亮度细节、颜色降噪和颜色细节。
- 暗角、颗粒等基础效果。

#### 几何与镜头

- 自由及固定比例裁剪、旋转、翻转和拉直。
- 垂直、水平、旋转、比例和偏移透视校正。
- Lensfun 自动匹配镜头。
- 畸变、暗角、横向色差和紫边校正。
- 约束裁剪及手动缩放。

#### 图像堆栈与拼接

- 欢迎页提供统一“图像堆栈”入口，可一次选择 2–200 张受支持的 RAW 或位图；进入工作台后再选择景深堆栈或全景拼接，不在首页重复暴露工作流按钮。超过 30 张时自动使用低内存大批量路径，候选关系同时参考用户顺序、文件名自然顺序和紧凑视觉检索，并对水墨留白等低纹理画面补充低阈值特征检测，避免文件选择器乱序、焦段切换或角点过少造成整组断连。
- 堆栈工作台使用无外层留白的全窗口布局，预览画布占据主要空间，工作流、对齐模型和源图层集中在右侧；源图层支持鼠标拖拽、键盘拖拽及上下按钮调整基准顺序。
- 自动对齐优先在相似与仿射模型之间稳健选择；30 张以内继续使用双向 RANSAC、全分辨率局部相关搜索和亚像素峰值细化，超过 30 张则在自适应分辨率的多尺度灰度分析图上完成匹配，并用 35mm 等效焦段元数据补充尺度桥接，再按需逐张解码原图参与最终融合。大批量自动全景会根据平移、旋转、缩放和拟合误差选择证据充分的模型，避免误把重复纹理或微小误差累积到长链中；明确改变机位时仍可选择透视模型。
- 支持自动、透视、圆柱、球面和仅位置对齐模型；自动结果不稳定时可以切换模型并重新对齐。
- 同机位景深堆栈使用局部清晰度评分与多尺度频带融合；检测到明显取景、旋转或焦段变化时改用连续最优接缝，避免跨画面逐像素混合产生双影和模糊空洞。全景拼接对低频曝光与高频纹理分别融合，在平滑亮度过渡的同时保留边缘细节；无法建立连接的独立场景会明确提示，而不会静默生成错误的超大拼图。
- 2026-09-18 起，长幅平面画作的流式拼接会要求自动透视模型具有跨画面的空间支持，避免重复笔画触发整幅错误投影；低纹理画面使用局部对比特征检测，拍摄组间色调只从同机位重叠估计，最终所有权接缝按像素级双线性羽化，避免宽幅灰带和分析网格方块。参考验收流程及限制见 [`docs/focus-stack-reference-validation.md`](docs/focus-stack-reference-validation.md)。
- 合成结果使用缓存文件提供最高约一亿像素的高质量预览，支持滚轮缩放、拖拽平移、适合窗口、100% 查看和专注预览。全景原始画布超过 2.4 亿像素时，后端会按同一组全分辨率对齐变换进行内存安全渲染，并在预览角标显示缩放比例与完整画布估算，避免 32 GB 机器直接申请数十 GiB 浮点画布。预览与保存绑定同一个结果 ID，并共享规范的 16 位显示编码 sRGB 像素；导出的 TIFF 嵌入同一 sRGB ICC，避免浮点 TIFF 被查看器按线性 RGB 解释而造成显示差异，同时保留本地处理 sidecar 关联。

#### 非 AI 局部编辑

- 画笔、线性渐变、径向渐变。
- 颜色范围和明度范围。
- 蒙版的添加、减去、相交、反转、羽化和密度。
- 实时蒙版覆盖、单独显示和复制。
- 局部曝光、影调、白平衡、颜色、清晰度、去雾、锐化和降噪。
- Heal、Clone、尘点可视化和红眼修复。

#### 非破坏工作流

- 版本化 sidecar，任何时候都能重新打开并继续编辑。
- 撤销、重做、历史步骤、快照和虚拟副本。
- 用户预设、参数默认值和预设强度。
- Before/After、左右分屏、曝光裁切警告、直方图。
- 胶片栏、多选、复制/粘贴全部或部分设置。
- 批量应用设置和后台导出队列。
- JPEG、PNG 和 16 位 TIFF 导出；保留可选择的 EXIF、拍摄时间和 ICC。

### 暂不进入 1.0

- Android 移动端应用、APK/AAB 安装包和移动端发布流水线。
- AI Denoise、Raw Details、Super Resolution。
- 主体、天空、人物、背景、对象和深度 AI 蒙版。
- 反射移除、自动尘点识别、生成式移除和生成式扩图。
- Lens Blur、景深重建。
- HDR 合并。
- Photoshop 插件或 PSD 智能对象兼容。

这些本地功能只有在 Daily RAW 1.0 的 RAW 管线、导出一致性和日常稳定性达标后才进入规划。

### 不属于产品范围

- 账号、登录、订阅和用户画像。
- 社区、在线预设市场和社交功能。
- 云图库、云端 AI、在线同步和远程编辑服务。
- 上传照片、EXIF、编辑参数、提示词、使用数据或崩溃数据。

允许从公共源下载开源库、运行时、算法数据和模型，或随安装包离线分发；这类资源只服务于
本机计算，不改变上述产品边界。

## 完成标准

Daily RAW 1.0 只有同时满足以下条件才算完成：

- 原始 RAW 文件在全部编辑、退出、崩溃恢复和导出流程中保持字节不变。
- 关闭应用后重新打开，sidecar 能完整恢复全局参数、裁剪、蒙版、历史关键点和输出设置。
- 预览与全分辨率导出不存在明显的颜色、裁剪、蒙版边缘或锐化差异。
- α7R V 60MP ARW 可以完成连续导入、编辑、切换、100% 查看和导出，不崩溃、不产生空白图。
- 常用滑块在指定测试 Mac 上以交互预览运行，目标为 P95 更新延迟不超过 33ms。
- 使用嵌入缩略图时首屏目标小于 1 秒；可编辑预览目标小于 3 秒。
- 缓存后的相邻照片切换目标小于 500ms。
- 批量导出 100 张混合 RAW 时，单张失败不会中止队列，并能看到文件级错误和重试入口。
- JPEG 和 TIFF 带正确 ICC；重新打开导出文件后颜色与应用预览一致。
- macOS 发布构建通过；Windows、Linux 至少通过 CI 构建和核心渲染回归测试。
- 不需要网络即可完成 1.0 的全部编辑功能。

性能数字是工程预算，不是牺牲画质的理由。如果目标机器或测试素材变化，需要在基准报告中
记录新条件，而不是静默降低输出质量。

## 技术方向

沿用并强化 RapidRAW 已验证的技术路线：

- 应用壳：Tauri 2。
- 前端：React、TypeScript、Vite。
- 核心：Rust，耗时任务放入后台工作线程，禁止阻塞 UI。
- GPU：WGPU + WGSL，使用 32 位 scene-linear 处理。
- RAW：保留可替换的解码器接口；先使用 RapidRAW/rawler 现有能力，必要时补充 LibRaw
  或 RawSpeed 后备。
- 镜头：Lensfun。
- ICC：优先接入 LittleCMS。
- 数据：版本化 JSON 编辑数据统一保存在应用私有数据目录；SQLite 保存索引、缩略图、评分和队列状态，不在照片目录生成内部文件。
- 输出：原生分块渲染和编码，不通过 WebView IPC、JSON 或 Base64 传输整张图。
- AI：1.0 默认不启用；后续能力只接受公开模型的可选下载或本地打包，并且只在本机推理。

处理管线必须保持明确顺序，不能继续演变成一个难以验证的超大 shader：

```text
RAW 解包
  -> 黑白电平 / 坏点
  -> 镜头预校正和色差
  -> 白平衡
  -> 去马赛克
  -> 高光恢复
  -> 相机配置文件
  -> scene-linear 工作空间
  -> 全局影调与颜色
  -> 局部蒙版与修复
  -> 输出效果和锐化
  -> 显示 ICC 或导出 ICC
```

每个节点都必须声明：

- 输入、输出颜色空间和数值范围；
- 是否依赖完整分辨率或邻域像素；
- 是否支持分块和 ROI；
- 是否能在 GPU 和参考实现之间做结果对照；
- 参数版本以及 sidecar 迁移规则。

## 参考项目与使用边界

| 项目                                                                    | 用途                                              | 使用方式                     |
| ----------------------------------------------------------------------- | ------------------------------------------------- | ---------------------------- |
| [RapidRAW](https://github.com/CyberTimon/RapidRAW)                      | 应用基础、WGPU 管线、蒙版、sidecar、图库和批处理  | 直接 fork，持续跟踪上游      |
| [darktable](https://github.com/darktable-org/darktable)                 | scene-linear 管线、模块顺序、蒙版、缓存和回归思路 | 研究并按许可证记录可复用部分 |
| [RawTherapee](https://github.com/RawTherapee/RawTherapee)               | 去马赛克、高光恢复、锐化、降噪、细节处理          | 算法和画质参考               |
| [ART](https://github.com/artraweditor/ART)                              | 在专业能力下控制 UI 复杂度                        | 产品和交互参考               |
| [LibRaw](https://github.com/LibRaw/LibRaw)                              | RAW 格式兼容性和元数据                            | 作为候选后备解码器           |
| [RawSpeed](https://github.com/darktable-org/rawspeed)                   | 高速 RAW 解包和 CFA/黑电平                        | 作为候选解包层               |
| [Lensfun](https://github.com/lensfun/lensfun)                           | 畸变、暗角和横向色差                              | 直接依赖并保留数据库许可信息 |
| [LittleCMS](https://github.com/mm2/Little-CMS)                          | ICC v2/v4 色彩管理                                | 计划直接依赖                 |
| [OpenColorIO](https://github.com/AcademySoftwareFoundation/OpenColorIO) | ACES、LUT 和高级颜色空间                          | 1.0 后评估                   |
| [OpenImageIO](https://github.com/AcademySoftwareFoundation/OpenImageIO) | 专业格式输入输出                                  | TIFF/EXR/JXL 需求扩大时评估  |

本项目整体继续使用 AGPL-3.0。任何复制、移植或改写外部源码的工作开始前，都必须先把
项目、文件、原许可证、修改内容和版权声明加入第三方许可证清单。不能因为整体已经是
AGPL 就省略来源和原作者声明。发布二进制时必须同步提供对应源代码和构建说明。

## 里程碑

### M0：建立可维护的上游分支

预计：1 周。

工作内容：

- Fork RapidRAW，配置 `origin` 为本项目、`upstream` 为 RapidRAW 官方仓库。
- 记录起始 tag、commit、依赖版本和当前许可证。
- 在 macOS 完成未修改上游版本的开发与发布构建。
- 用 α7R V ARW、DNG、JPEG 建立第一批冒烟素材。
- 运行并记录现有导入、基础调整、蒙版、Lensfun、sidecar 和导出的实际表现。
- 建立“保留、修改、暂时隐藏、1.0 后再做”的功能清单。

验收：

- 未修改上游基线可以稳定启动和导出。
- 所有基准素材都有结果、耗时、峰值内存和已知问题记录。
- README、AGPL、NOTICE/第三方许可证和上游提交信息完整。

### M1：锁定 RAW 与色彩基线

预计：2–3 周。

工作内容：

- 审计从 RAW 解包到显示变换的每个处理节点和颜色空间。
- 优先修复 α7R V ARW 的黑电平、白平衡、方向、色彩、裁剪区域和高光恢复。
- 建立 DNG、CR3、NEF、RAF 的最小兼容性冒烟测试。
- 引入或完善显示 ICC 和导出 ICC。
- 建立快速预览、半分辨率编辑、100% ROI 和全分辨率导出四级渲染策略。
- 消除整图 Base64、重复 CPU/GPU 缓冲和无界缓存。

验收：

- 同一参数下，预览、100% ROI 和导出没有可见色差。
- α7R V 日光、钨丝灯、高 ISO、欠曝、高饱和光源样片全部通过人工画质检查。
- 60MP 文件达到完成标准中的首屏、编辑预览和稳定性预算。

### M2：完成全局编辑工作流

预计：4–5 周。

工作内容：

- 对齐基础影调、曲线、HSL、颜色分级、黑白和效果面板。
- 完成锐化、亮度降噪和颜色降噪的 100% 预览。
- 完成 Lensfun 自动匹配、手动匹配、畸变、暗角、色差和紫边。
- 完成裁剪、拉直、旋转、透视和约束裁剪。
- 统一数值输入、双击复位、面板开关、参数局部重置和键盘操作。
- 确保关闭某模块时真正绕过其计算节点。

验收：

- 能只依靠全局工具完成日光、室内、人像、风景、夜景和高 ISO 六类样片。
- 任意参数可撤销、重做、复制、粘贴、保存为预设并在重启后恢复。
- 关闭与开启模块的差异可通过 Before/After 和回归导出验证。

### M3：完成非 AI 局部编辑

预计：4–5 周。

工作内容：

- 稳定画笔、线性渐变、径向渐变。
- 增加颜色范围、明度范围、添加、减去、相交和反转。
- 统一蒙版坐标与裁剪、旋转、透视、镜头校正之间的变换关系。
- 完成蒙版羽化、密度、流量、覆盖显示和独显。
- 完成 Heal、Clone、尘点可视化和红眼。
- 为蒙版建立缩略缓存、ROI 重绘和边缘回归测试。

验收：

- 蒙版在裁剪、旋转、镜头校正、复制到其他图片和重新打开后位置不漂移。
- 60MP 图像上连续绘制不会阻塞 UI，缩放到 100% 后边缘无明显断裂。
- 局部调整预览和导出一致。

### M4：完成非破坏与批量工作流

预计：3–4 周。

工作内容：

- 维护已冻结的 sidecar 1.0 schema、版本迁移、原子写入和崩溃恢复测试。
- 完成历史、快照、虚拟副本和每相机默认值。
- 完成胶片栏、多选、部分参数复制、同步及撤销。
- 完成后台导出队列、暂停、取消、重试和文件级错误。
- 增加导出预设、重命名、目录结构、EXIF、ICC 和输出锐化选项。
- 增加崩溃恢复和未完成任务恢复。

验收：

- 100 张混合 RAW 批量同步和导出不会阻塞下一张照片的编辑。
- 单张失败不会影响其他任务，错误能够定位到具体文件。
- 旧 sidecar 样本在新版本中可迁移且不改变渲染结果。

### M5：稳定性、平台和 Daily RAW 1.0

预计：3–4 周。

工作内容：

- 完成 macOS、Windows、Linux 的构建、签名和安装包流程。
- 建立 GPU 后端兼容矩阵和安全降级路径。
- 完成内存泄漏、快速切图、超大图、损坏文件和低磁盘空间测试。
- 完成键盘导航、焦点、屏幕阅读器标签、对比度和缩放可访问性检查。
- 完成许可证、隐私、安全、性能和依赖审计。
- 编写用户文档、已知限制、支持机型和故障排查。

验收：

- 全部“完成标准”通过。
- 连续完成至少三次真实拍摄批次的导入、挑选、编辑和导出。
- 不存在 P0/P1 数据损坏、导出错误、颜色错误、崩溃或编辑丢失问题。

## 日常开发流程

### 1. 开始一个功能

1. 从 `main` 创建短生命周期分支：`codex/<milestone>-<feature>`。
2. 在 issue 或任务说明中写清用户场景、输入、输出、非目标和验收样片。
3. 明确处理节点位置、颜色空间、分辨率层级、sidecar 字段和许可证影响。
4. 先补测试或基准，再实现功能。

### 2. 实现

1. 参数 schema、Rust 核心、WGSL、前端控件和 sidecar 序列化必须作为一个闭环实现。
2. 预览不得采用导出时无法复现的临时算法。
3. UI 只传参数、任务状态和小型统计数据；完整像素留在原生内存或 GPU。
4. 所有长任务必须可取消，并提供进度、文件名和明确错误。
5. 修改管线顺序、色彩空间或默认参数时，必须增加 sidecar 版本或迁移规则。

### 3. 验证

每个图像处理变更至少完成：

- 单元测试：参数边界、矩阵、坐标变换、sidecar round-trip。
- GPU/CPU 或参考输出对照。
- 小图、60MP 图、横竖方向、裁剪、旋转和带蒙版导出。
- α7R V 基准样片人工检查。
- 性能记录：首屏、可编辑预览、滑块延迟、100% ROI、导出、峰值内存。
- `git diff --check`、Rust 格式化、TypeScript 检查、lint 和测试。

不能只凭编译成功或缩略图正常就宣布修复。涉及导出的修改必须检查实际导出文件；涉及
高分辨率性能的修改必须使用真实 60MP RAW。

### 4. 合并

合并前必须审查：

- 功能逻辑和边界情况；
- 预览/导出一致性；
- 性能和内存；
- 可访问性和响应式布局；
- 文件路径、元数据和不可信 RAW 的安全处理；
- 许可证和第三方声明；
- 是否意外改变旧 sidecar 或默认成像结果。

`main` 必须始终保持可构建、可启动、可打开基准 RAW、可导出。上游 RapidRAW 的更新只在
里程碑边界集中同步；同步后先完整跑基准，再开始下一个功能，避免开发中途持续漂移。

## 测试素材与画质基线

测试集至少包含：

- α7R V：日光、阴天、钨丝灯、混合光、人像肤色、红色展品、蓝紫灯光。
- ISO 100、ISO 1600、ISO 6400 及更高 ISO。
- 正常曝光、欠曝 2–4 档、局部高光剪切。
- 高反差建筑、细密纹理、树叶、织物、星点和重复图案。
- 明显桶形/枕形畸变、暗角、横向色差和紫边。
- Bayer、X-Trans、DNG，以及损坏或截断文件。
- 横图、竖图、不同裁剪、旋转和多层蒙版。

大型 RAW 不直接提交到 Git 仓库。仓库保存素材清单、相机信息、许可、校验值、预期输出
版本和获取方式。基准导出需要记录处理引擎版本、GPU、操作系统和参数 sidecar。

画质对照使用 Camera Raw、darktable 和 RawTherapee 的代表性导出，但不以单一 SSIM/PSNR
作为正确标准。检查内容包括中性灰、肤色、饱和色、高光过渡、暗部色偏、细节、噪声、
摩尔纹、锐化光晕、镜头边缘和蒙版边界。

## 发布原则

- 推送到 `main` 后自动构建全部桌面安装包并创建 GitHub Release；同一版本线按补丁号递增，
  例如 `v0.1.0`、`v0.1.1`、`v0.1.2`。
- 自动版本以仓库当前版本为起点，只递增补丁号。进入新的版本线（例如 `1.0.0`）时，运行
  `npm run version:set -- 1.0.0`，检查并提交版本文件后再推送。
- 原片安全优先于功能数量。
- 预览和导出一致优先于单纯追求预览速度。
- 真实相机样片优先于合成 demo。
- 非 AI 核心稳定前不扩张本地 AI；云端、账号和社区能力不进入产品规划。
- 不复制 Camera Raw 的品牌、图标或界面；对齐的是工作流和摄影能力。
- 每次发布附带支持机型、已知限制、性能基准、迁移说明和完整对应源代码。
