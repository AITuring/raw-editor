...
assert.match(maskGeneration, /fn decode_ai_bitmap_from_base64\(/);
assert.match(maskGeneration, /struct TiledAiMaskRasterizer/);
assert.match(maskGeneration, /Ai\(TiledAiMaskRasterizer\)/);
assert.match(maskGeneration, /"ai-subject" \| "ai-foreground" \| "ai-sky" \| "ai-depth" \| "quick-eraser"/);
assert.match(maskGeneration, /fn image_gaussian_blur_radius\(/);
assert.match(maskGeneration, /selection\.apply_blur\(mask\)/);
...
const rgbaF16Conversion = gpuProcessing.slice(rgbaF16Start, rgbaF16End);
assert.match(rgbaF16Conversion, /DynamicImage::ImageRgb32F\(rgb\)/);
assert.match(rgbaF16Conversion, /DynamicImage::ImageRgba32F\(rgba\)/);
assert.doesNotMatch(rgbaF16Conversion, /let rgba_f32 = img\.to_rgba32f\(\)/);
const rawHandoffStart = rawProcessing.indexOf('fn developed_intermediate_into_dynamic_image(');
const rawHandoffEnd = rawProcessing.indexOf('\nfn develop_internal(', rawHandoffStart);
assert.ok(rawHandoffStart >= 0 && rawHandoffEnd > rawHandoffStart);
const rawHandoff = rawProcessing.slice(rawHandoffStart, rawHandoffEnd);
assert.match(rawHandoff, /pixels\.into_inner\(\)\.into_flattened\(\)/);
assert.match(rawHandoff, /DynamicImage::ImageRgb32F\(buffer\)/);
assert.doesNotMatch(rawHandoff, /ImageRgba32F/);
const rawEnhancementStart = imageProcessing.indexOf('pub fn remove_raw_artifacts_and_enhance(');
const rawEnhancementEnd = imageProcessing.indexOf('\nfn enhance_rgb32f_buffer(', rawEnhancementStart);
assert.ok(rawEnhancementStart >= 0 && rawEnhancementEnd > rawEnhancementStart);
assert.match(imageProcessing.slice(rawEnhancementStart, rawEnhancementEnd), /image\.as_mut_rgb32f\(\)/);
const rawDisplayStart = imageProcessing.indexOf('pub fn apply_cpu_default_raw_processing(');
const rawDisplayEnd = imageProcessing.indexOf('\nfn apply_cpu_default_raw_processing_to_rgb32f(', rawDisplayStart);
assert.ok(rawDisplayStart >= 0 && rawDisplayEnd > rawDisplayStart);
assert.match(imageProcessing.slice(rawDisplayStart, rawDisplayEnd), /image\.as_mut_rgb32f\(\)/);
...
const maxAiDepthMaskHalo = 32 + maxRangeMaskHalo;
const maxAiDepthMaskTileBytes = (2048 + maxAiDepthMaskHalo * 2) ** 2;
...
assert.equal(rawRgb32fBytes, 722_608_128);
...
assert.equal(maxAiDepthMaskHalo, 159);
assert.equal(maxAiDepthMaskTileBytes, 5_597_956);
assert.equal(activeMaskFullTextureBytes * 3 - maxAiDepthMaskTileBytes * 3, 163_858_164);
